...
Supports both direct commands and complex instruction conversion
"""
...
Respond ONLY with a JSON array of Minecraft commands."""
    ...
    def run_interactive(self):
        """Run interactive mode with support for both direct commands and complex instructions"""
        print("Minecraft AI Bot (Herdora) started. Type instructions:")
        print("(Press Ctrl+C to exit)")
        print("\nModes:")
        print("  - Direct commands: /tp @p 100 64 100, /fill ~ ~ ~ ~5 ~5 ~5 stone")
        print("  - Complex instructions: Build a house, Create a bridge, Make a tower")
        ...
        while True:
            try:
                ...
                if user_input.startswith("/"):
                    # Direct command - send as is
                    print(f"Executing direct command: {user_input}")
                    success = self.send_to_mcp(user_input)
                    if success:
                        print("Command sent to MCP server")
                else:
                    # Complex instruction - convert to commands
                    print(f"Converting complex instruction: '{user_input}'")
                    commands = self.convert_instruction_to_commands(user_input)
                    ...
                    if commands:
                        ...
                        for i, cmd in enumerate(commands, 1):
                            ...
                            success = self.send_to_mcp(cmd)
                            if success:
                                print(f"Command {i} sent to MCP server")
                            else:
